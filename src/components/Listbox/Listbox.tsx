...
const Button = ({
  ...
  return (
    <Listbox.Button
      ...
      className="relative w-full rounded-lg bg-white h-10 pl-4 pr-10 text-left border border-slate-200"
      {...props}
    >
      ...
      <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-2">
        <SelectorIcon className="h-5 w-5 text-slate-300" aria-hidden="true" />
      </span>
    </Listbox.Button>
  );
};
    ...
    <Listbox.Option
      className={({ active }) =>
        `relative cursor-pointer select-none py-2 pl-10 pr-4 ${
          active ? "bg-cyan-50 text-cyan-600" : "text-slate-600"
        }`
      }
      {...props}
    >
